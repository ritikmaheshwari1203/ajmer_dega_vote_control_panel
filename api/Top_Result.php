    ...
    if($numrows>0){
        ...
        for ($i=0; $i <count($file_id) ; $i++) { 
            if($teacher_name[$i][1]=='NULL'){
                ...
                $q=$teacher_id[$i][0];
                $sqli = "SELECT * FROM `student_teacher_mapping` WHERE `student_id` = $file_id[$i] AND `teacher_id`!=$q";
                $res = mysqli_query($conn, $sqli);
            ...
            if ($teacher_name[$i][2]=='NULL') {
                ...
                $q=$teacher_id[$i][0];
                $w=$teacher_id[$i][1];
                $sqli = "SELECT * FROM `student_teacher_mapping` WHERE `student_id` = $file_id[$i] AND `teacher_id`!=$q AND `teacher_id`!=$w ";
                $res = mysqli_query($conn, $sqli);
                $count = mysqli_num_rows($res);
                // $j=0;
                // $z=1;
        ...
        // $marks,$file_id,$teacher_id

        $output = array($marks,$file_id,$teacher_name);
